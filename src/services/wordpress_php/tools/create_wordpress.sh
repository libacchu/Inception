...
then 
	...
	sed -i "s/database_name_here/$MARIADB_DATABASE/g" /var/www/html/wordpress/wp-config-sample.php
	sed -i "s/username_here/$MARIADB_USER/g" /var/www/html/wordpress/wp-config-sample.php
	sed -i "s/password_here/$MARIADB_PASSWORD/g" /var/www/html/wordpress/wp-config-sample.php
	...
	# wp user create --allow-root ${WORDPRESS_DB_USER} \
	# 		${WORDPRESS_EMAIL} \
	# 		--role=contributor \
